...
echo "  G9.9  关窗隐藏 / 托盘退出 / 托盘菜单代理开关、模式切换、订阅切换与节点切换（多层子菜单）/ 图标右侧的实时速度读数（上下两行，代理服务运行时每秒刷新；关闭后显示 00.0 而不消失并转为淡灰色，仅内核单独运行时同样置灰）/ 设置里的「系统托盘图标」三种显示模式（图标+网速、仅图标、仅网速）即时生效并持久化 — 需 GUI，请 npm run dev 后目视确认"
