...
pub fn maybe_decode_base64(raw: &str) -> Result<String, SubscriptionError> {
    let trimmed = raw.trim();
    if trimmed.starts_with('{') || trimmed.to_ascii_lowercase().contains("proxies:") {
        ...
        .or_else(|_| base64::engine::general_purpose::URL_SAFE_NO_PAD.decode(&compact))
    {
        Ok(bytes) => match String::from_utf8(bytes) {
            ...
            Ok(_) | Err(_) => Ok(trimmed.to_string()),
        },
