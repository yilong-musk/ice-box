...
Status: **proposal — shared/macOS T0 complete, T1–T4 shared complete, T5 macOS helper + packaging landed, macOS live orchestration gates G9.12 and G9.13 green; macOS is a permanently unsigned release with in-app elevated helper installation (system authorization dialog), the clean-machine gate is explicitly waived for this release, and Windows T0 is complete — `windows_tun_ready` flipped 2026-09-03** (the Windows TUN backend — `WindowsTunBackend`, `netsh`/`route print`/DNS probes with host-free parsing tests, graceful-stop elevated runner, DNS ownership with compare-before-restore — is the production path; the gate flipped behind the §1.2 config shape in `docs/design-notes/tun-windows-t0.md`, verified live on a real Windows 11 host with the pinned 1.13.19; Windows T1–T5 now track the macOS slice order).
