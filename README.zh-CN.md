...
自启项按用户注册——macOS 是 `~/Library/LaunchAgents` 下的 LaunchAgent plist，Windows 是 `HKCU` 下的 `Run` 项——每次启动都会重新指向当前的安装位置。macOS 13 及以上会在 **系统设置 → 通用 → 登录项** 中把它列为后台项目，也可以在那里关闭；关闭 ice-box 里的开关会直接删除该项。
